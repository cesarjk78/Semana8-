...
export default function Productos() {
  ...
  return (
    <Container className="mt-5">
      ...
      {error && <Alert variant="info" className="text-primary">{error}</Alert>}
          ...
          <Col key={p.id}>
            <Card className="h-100 shadow">
              <Card.Body>
                ...
                {usuario.rol !== 'usuario' && (
                  <>
                    <Button variant="info" size="sm" onClick={() => editar(p)} className="me-2 text-white">
                      ✏️ Editar
                    </Button>
                    {usuario.rol === 'admin' && (
      ...
      {usuario.rol !== 'usuario' && (
        <Form onSubmit={editando ? actualizarProducto : crearProducto} className="bg-light p-4 rounded shadow">
          <h4 className="text-center text-primary">{editando ? 'Editar Producto' : 'Nuevo Producto'}</h4>
